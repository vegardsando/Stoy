...
class LabelEnvironmentPlugin extends BasePlugin
{
    ...
    function init() {
        ...
        if ( craft()->request->isCpRequest() && craft()->userSession->isLoggedIn() )
        {
            ...
            craft()->templates->includeCss('#global-sidebar:before { content: "' . $pluginSettings->prefix . ' ' . $environmentName . ' ' . $pluginSettings->suffix . '"; }');

            if (isset($pluginSettings->colorMappings[$environmentName]))
            {
                craft()->templates->includeCss('#global-sidebar:before { background-color: '. $pluginSettings->colorMappings[$environmentName] . '; }');
            }
